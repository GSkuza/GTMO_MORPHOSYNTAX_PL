...
""")
    ...
    try:
        # Test JSON saver
        from gtmo_json_saver import GTMOJSONSaver
        
        saver = GTMOJSONSaver(output_dir="test_output")
        ...
        output_path = saver.save_single_result(test_result, "test_result.json")
        print(f"✓ Saved test result to: {output_path}")
        
        # Verify saved file
        with open(output_path, 'r', encoding='utf-8') as f:
            loaded = json.load(f)
            if 'result' in loaded and 'coordinates' in loaded['result']:
                print("✓ JSON save/load verification passed")
            else:

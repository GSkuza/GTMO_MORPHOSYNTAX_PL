...
1) dyrektywę Parlamentu Europejskiego i Rady (UE) 2020/1828 z dnia 25 listopada 2020 r.
...
1) nabycie i zbycie oraz obciążanie nieruchomości, użytkowania wieczystego lub
...
1) własność nieruchomości lub ruchomości, w tym urządzeń, materiałów, towarów
...
1) prawa wynikające z umów najmu i dzierżawy nieruchomości lub ruchomości
...
1) skutkach prawnych potwierdzenia przez drugą stronę otrzymania oferty;
1) zasadach i sposobach utrwalania, zabezpieczania i udostępniania przez

przedsiębiorcę drugiej stronie treści zawieranej umowy;

1) metodach i środkach technicznych służących wykrywaniu i korygowaniu błędów

we wprowadzanych danych, które jest obowiązany udostępnić drugiej stronie;

1) językach, w których umowa może być zawarta;
1) kodeksach etycznych, które stosuje, oraz o ich dostępności w postaci
...
1) w razie umieszczenia na obejmującym czynność dokumencie jakiejkolwiek

wzmianki przez organ państwowy, organ jednostki samorządu terytorialnego

albo przez notariusza – od daty wzmianki;

1) w razie opatrzenia kwalifikowanym elektronicznym znacznikiem czasu
...
. Prokura może obejmować umocowanie także albo wyłącznie do
...
. Prokura wygasa wskutek ustanowienia kuratora na podstawie art. 42 § 1. W
...
prokury łącznej oraz prokury, o której mowa w art. 1094
...
1) długość okresu od upływu terminu przedawnienia do chwili dochodzenia

roszczenia;

1) charakter okoliczności, które spowodowały niedochodzenie roszczenia przez
...
1) co do roszczeń, które przysługują osobom niemającym pełnej zdolności do
...
1) co do roszczeń, które przysługują jednemu z małżonków przeciwko drugiemu –

przez czas trwania małżeństwa;

1) co do wszelkich roszczeń, gdy z powodu siły wyższej uprawniony nie może ich
...
1) co do roszczeń objętych wnioskiem o zawezwanie do próby ugodowej – przez
...
własności potrzebne jest dodatkowe porozumienie stron obejmujące ich bezwarunkową zgodę na niezwłoczne przejście własności.
...
niemożności wezwania – w ciągu dwóch lat od dnia jej znalezienia, staje się własnością znalazcy, jeżeli uczynił on zadość swoim obowiązkom. Jeżeli jednak rzecz
...
1) sytuację osobistą i majątkową współwłaściciela zobowiązanego do spłat
...
1) rodzaj budynków lub urządzeń oraz obowiązek ich utrzymywania w należytym

stanie;

1) warunki i termin odbudowy w razie zniszczenia albo rozbiórki budynków lub

urządzeń w czasie trwania użytkowania wieczystego;

1) wynagrodzenie należne wieczystemu użytkownikowi za budynki lub urządzenia
...
służebność zwiększa użyteczność tylko jednej lub kilku z nich, właściciel nieruchomości obciążonej może żądać zwolnienia jej od służebności względem części
...
- ustanowienie służebności przesyłu, a jest ona konieczna dla właściwego korzystania
...
z nieruchomości. Jeżeli powodowałoby to nadmierne trudności lub koszty, przedsiębiorca jest obowiązany do naprawienia wynikłej stąd szkody.
...
- rozwiązaniu umowy. Rozwiązując umowę sąd może w miarę potrzeby orzec
...
1) wyłączają lub istotnie ograniczają potrącenie wierzytelności konsumenta

z wierzytelnością drugiej strony;

1) przewidują postanowienia, z którymi konsument nie miał możliwości zapoznać

się przed zawarciem umowy;

1) zezwalają kontrahentowi konsumenta na przeniesienie praw i przekazanie

obowiązków wynikających z umowy bez zgody konsumenta;

1) uzależniają zawarcie umowy od przyrzeczenia przez konsumenta zawierania

w przyszłości dalszych umów podobnego rodzaju;

©Kancelaria Sejmu s. 76/242

2025-09-09

1) uzależniają zawarcie, treść lub wykonanie umowy od zawarcia innej umowy,

niemającej bezpośredniego związku z umową zawierającą oceniane

postanowienie;

1) uzależniają spełnienie świadczenia od okoliczności zależnych tylko od woli

kontrahenta konsumenta;

1) przyznają kontrahentowi konsumenta uprawnienia do dokonywania wiążącej

interpretacji umowy;

1) uprawniają kontrahenta konsumenta do jednostronnej zmiany umowy bez ważnej

przyczyny wskazanej w tej umowie;

1) przyznają tylko kontrahentowi konsumenta uprawnienie do stwierdzania

zgodności świadczenia z umową;

1) wyłączają obowiązek zwrotu konsumentowi uiszczonej zapłaty za świadczenie

niespełnione w całości lub części, jeżeli konsument zrezygnuje z zawarcia

umowy lub jej wykonania;

1) przewidują utratę prawa żądania zwrotu świadczenia konsumenta spełnionego

wcześniej niż świadczenie kontrahenta, gdy strony wypowiadają, rozwiązują lub

odstępują od umowy;

1) pozbawiają wyłącznie konsumenta uprawnienia do rozwiązania umowy,

odstąpienia od niej lub jej wypowiedzenia;

1) zastrzegają dla kontrahenta konsumenta uprawnienie wypowiedzenia umowy

zawartej na czas nieoznaczony, bez wskazania ważnych przyczyn i stosownego

terminu wypowiedzenia;

1) nakładają wyłącznie na konsumenta obowiązek zapłaty ustalonej sumy na

wypadek rezygnacji z zawarcia lub wykonania umowy;

1) nakładają na konsumenta, który nie wykonał zobowiązania lub odstąpił od

umowy, obowiązek zapłaty rażąco wygórowanej kary umownej lub odstępnego;

1) stanowią, że umowa zawarta na czas oznaczony ulega przedłużeniu, o ile

konsument, dla którego zastrzeżono rażąco krótki termin, nie złoży przeciwnego

oświadczenia;

1) przewidują wyłącznie dla kontrahenta konsumenta jednostronne uprawnienie do

zmiany, bez ważnych przyczyn, istotnych cech świadczenia;

©Kancelaria Sejmu s. 77/242

2025-09-09

1) przewidują uprawnienie kontrahenta konsumenta do określenia lub

podwyższenia ceny lub wynagrodzenia po zawarciu umowy bez przyznania

konsumentowi prawa odstąpienia od umowy;

1) uzależniają odpowiedzialność kontrahenta konsumenta od wykonania
...
1) przewidują obowiązek wykonania zobowiązania przez konsumenta mimo

niewykonania lub nienależytego wykonania zobowiązania przez jego

kontrahenta;

1) wyłączają jurysdykcję sądów polskich lub poddają sprawę pod rozstrzygnięcie
...
. § 1. Przepisy dotyczące konsumenta, zawarte w art. 3851
...
1) wartość zabezpieczanych tą nieruchomością roszczeń pieniężnych nie jest

oznaczona, lub

1) zawarcie tej umowy nie zostało poprzedzone dokonaniem wyceny wartości
...
. Jeżeli wartość świadczenia jednej ze stron w chwili zawarcia umowy
...
1) jeżeli spełnienie świadczenia czyni zadość zasadom współżycia społecznego;
1) jeżeli świadczenie zostało spełnione w celu zadośćuczynienia przedawnionemu

roszczeniu;

1) jeżeli świadczenie zostało spełnione, zanim wierzytelność stała się wymagalna.
...
1) Z dniem 18 grudnia 2001 r. na podstawie wyroku Trybunału Konstytucyjnego z dnia 4 grudnia

2001 r. sygn. akt SK. 18/2000 (Dz. U. poz. 1638).

©Kancelaria Sejmu s. 84/242
...
zobowiązania, nie wyłącza roszczenia o naprawienie szkody z tytułu czynu niedozwolonego, chyba że z treści istniejącego uprzednio zobowiązania wynika co
...
stosownie do potrzeb poszkodowanego oraz do możliwości zarobkowych i majątkowych zmarłego przez czas prawdopodobnego trwania obowiązku alimentacyjnego.
...
w szczególności wypadku, gdy poszkodowany stał się inwalidą, a przyznanie jednorazowego odszkodowania ułatwi mu wykonywanie nowego zawodu.
...
©Kancelaria Sejmu s. 90/242

2025-09-09

§ 2. W przypadkach określonych w art. 445 § 1 i 2 oraz art. 4462
...
domniemanie, że spełnione zostały również świadczenia okresowe wymagalne wcześniej.
...
1) jeżeli wierzyciel nie ma pełnej zdolności do czynności prawnych ani

przedstawiciela uprawnionego do przyjęcia świadczenia;

1) jeżeli powstał spór, kto jest wierzycielem;
1) jeżeli z powodu innych okoliczności dotyczących osoby wierzyciela świadczenie
...
1) wierzytelności o dostarczenie środków utrzymania;
1) wierzytelności wynikające z czynów niedozwolonych;
1) wierzytelności, co do których potrącenie jest wyłączone przez przepisy
...
1) jeżeli przysługuje jej prawo, przed którym spłacona wierzytelność ma

pierwszeństwo zaspokojenia;

1) jeżeli działa za zgodą dłużnika w celu wstąpienia w prawa wierzyciela; zgoda

dłużnika powinna być pod nieważnością wyrażona na piśmie;

1) jeżeli to przewidują przepisy szczególne.
...
odstąpić. Przepisy art. 492, art. 4921
...
i w sposób przyjęty przy przesyłkach tego rodzaju; jeżeli stwierdził, że w czasie przewozu nastąpił ubytek lub uszkodzenie rzeczy, obowiązany jest dokonać wszelkich
...
1) dokonać sam oznaczenia i podać je do wiadomości kupującego wyznaczając mu
...
1) nie nadaje się do celu, o którym kupujący poinformował sprzedawcę przy
...
z publicznych zapewnień, o których mowa w art. 5561
...
. Z zastrzeżeniem art. 5611

§ 2 koszty wymiany lub naprawy ponosi
...
- odstąpieniu od umowy albo obniżeniu ceny z powodu wady rzeczy sprzedanej.
...
lub spełnienie innego świadczenia, sprzedawca może zwolnić się od odpowiedzialności z tytułu rękojmi, zwracając kupującemu zapłaconą sumę lub wartość
...
©Kancelaria Sejmu s. 122/242

2025-09-09

1) nazwę i adres gwaranta;
1) opis procedury, której uprawniony ma przestrzegać, aby móc skorzystać z

gwarancji;

1) wskazanie rzeczy, których dotyczy gwarancja;
1) warunki gwarancji.
...
1) pomoc agrotechniczna i zootechniczna;

©Kancelaria Sejmu s. 130/242
...
wskutek okoliczności, za które producent odpowiedzialności nie ponosi, niedopełnienie tego obowiązku z winy producenta wyłącza możność powoływania się na te
...
. Przepisu § 1 nie stosuje się w przypadku, gdy inwestorem jest Skarb
...
natychmiastowego zapłacenia wszystkich przewidzianych w umowie a niezapłaconych rat, pomniejszonych o korzyści, jakie finansujący uzyskał wskutek ich

zapłaty przed umówionym terminem i wygaśnięcia umowy leasingu oraz umowy ze

zbywcą.

©Kancelaria Sejmu s. 150/242
...
. Jeżeli mimo upomnienia przez finansującego, w formie

dokumentowej, korzystający narusza obowiązki określone w art. 7097
...
1) opłaty związane z odroczeniem terminu spłaty pożyczki, jej nieterminową spłatą

albo koszty o podobnym charakterze,

1) koszty usług dodatkowych, w szczególności koszty ubezpieczeń, koszty
...
z działalnością gospodarczą lub zawodową tej osoby łączna wysokość pozaodsetkowych kosztów nie może przekraczać maksymalnej wysokości pozaodsetkowych
...
- maksymalnej wysokości pozaodsetkowych kosztów, także w razie dokonania
...
§ 1, biorący pożyczkę zobowiązuje się udzielić zabezpieczenia zwrotu
...
§ 1, nie może być wyższa od sumy kwoty pożyczki powiększonej

- wysokość odsetek maksymalnych obliczonych bezpośrednio od tej kwoty za okres,

©Kancelaria Sejmu s. 155/242
...
. Przed zawarciem umowy, o której mowa w art. 7202

§ 1, dający

pożyczkę informuje biorącego pożyczkę w sposób jednoznaczny i zrozumiały

- łącznej wysokości pozaodsetkowych kosztów, wysokości odsetek oraz kwocie
...
. § 1. W przypadku spłaty pożyczki, o której mowa w art. 7202
...
§ 2. W przypadku spłaty pożyczki, o której mowa w art. 7202

§ 1, przed

terminem określonym w umowie poniesione pozaodsetkowe koszty ulegają obniżeniu

- te koszty, które dotyczą okresu, o który skrócono czas obowiązywania umowy,
...
osoby stosuje się odpowiednio przepisy art. 7201
...
stosuje się odpowiednio także do nieuregulowanych
...
1) roszczenia z tytułu utrzymania, pielęgnowania, wychowania lub nauki, jeżeli
...
prowizja ta należy się zgodnie z art. 7611

poprzedniemu agentowi, chyba że
...
©Kancelaria Sejmu s. 167/242

2025-09-09

1) agent wypowiedział umowę, chyba że wypowiedzenie jest uzasadnione
...
1) agent za zgodą dającego zlecenie przeniósł na inną osobę swoje prawa
...
odbiegający na niekorzyść agenta od postanowień art. 7643
...
1) przy ubezpieczeniu osobowym – umówionej sumy pieniężnej, renty lub innego
...
- postanowieniach zawartej umowy oraz ogólnych warunków ubezpieczenia
...
gospodarczą lub zawodową ubezpieczonej osoby fizycznej, art. 3851
...
Przeniesienie tych praw wymaga zgody ubezpieczyciela, chyba że umowa ubezpieczenia lub ogólne warunki ubezpieczenia stanowią inaczej.
...
być wyższa od poniesionej szkody, zapłaty świadczenia w części przenoszącej wysokość szkody ubezpieczający może żądać tylko od tego ubezpieczyciela. W takim

©Kancelaria Sejmu s. 183/242
...
1) przy ubezpieczeniu następstw nieszczęśliwych wypadków – uszkodzenia ciała,
...
- którym mowa w art. 2 pkt 1 ustawy z dnia 18 listopada 2020 r. o doręczeniach
...
- doręczeniach elektronicznych, z zachowaniem terminu miesięcznego, jednakże nie
...
- doręczeniach elektronicznych, wysłanym nie później niż na 14 dni przed upływem
...
przedsiębiorcy składowego z tytułu umowy składu, z wyjątkiem roszczeń z tytułu niewidocznych uszkodzeń rzeczy, jeżeli składający, w ciągu siedmiu dni od odbioru,
...
1) gdy kto zrzeka się prawa, którego jeszcze nie nabył albo które nabył w taki
...
- zachowek oraz obowiązek wykonania zapisów zwykłych i poleceń, jak również inne
...
1) podstępem lub groźbą nakłonił spadkodawcę do sporządzenia lub odwołania

testamentu albo w taki sam sposób przeszkodził mu w dokonaniu jednej z tych

czynności;

1) umyślnie ukrył lub zniszczył testament spadkodawcy, podrobił lub przerobił jego

testament albo świadomie skorzystał z testamentu przez inną osobę

podrobionego lub przerobionego;

©Kancelaria Sejmu s. 208/242

2025-09-09

1) uporczywie uchylał się od wykonywania wobec spadkodawcy obowiązku
...
1) uporczywie uchylał się od wykonywania obowiązku pieczy nad spadkodawcą,
...
. Jeżeli dziecko któregokolwiek z dziadków spadkodawcy nie dożyło
...
1) przysposobiony i jego zstępni nie dziedziczą po krewnych przysposabiającego,

a krewni przysposabiającego nie dziedziczą po przysposobionym i jego

zstępnych;

©Kancelaria Sejmu s. 211/242

2025-09-09

1) rodzice przysposobionego nie dziedziczą po przysposobionym, a zamiast nich
...
1) pod wpływem błędu uzasadniającego przypuszczenie, że gdyby spadkodawca
...
1) niewidomy, głuchy lub niemy;
1) kto nie może czytać i pisać;
1) kto nie włada językiem, w którym spadkodawca sporządza testament;
1) skazany prawomocnie wyrokiem sądowym za fałszywe zeznania.
...
1) przedsiębiorstwo lub gospodarstwo rolne;
...
z zastrzeżeniem terminu, chyba że co innego wynika z treści testamentu lub z okoliczności.
...
. W sprawach nieuregulowanych w niniejszym rozdziale oraz
...
©Kancelaria Sejmu s. 222/242

2025-09-09

niezdolny do pracy albo jeżeli zstępny uprawniony jest małoletni – dwie trzecie wartości udziału spadkowego, który by mu przypadał przy dziedziczeniu ustawowym,
...
względem innych uprawnionych do zachowku tylko do wysokości nadwyżki przekraczającej jej własny zachowek.
...
1) dopuścił się względem spadkodawcy albo jednej z najbliższych mu osób
...
. Do zachowania terminu, o którym mowa w § 1, wystarcza złożenie przed
...
. Jeżeli złożenie oświadczenia o przyjęciu lub o odrzuceniu spadku wymaga
...
1) spadkobierca powinien jednocześnie oświadczyć, czy i jak spadek przyjmuje,

czy też go odrzuca;

©Kancelaria Sejmu s. 230/242

2025-09-09

1) do zachowania terminu, o którym mowa w art. 88 § 2, wystarcza złożenie przed
...
1) dane, o których mowa w art. 10311

§ 3,

1) imię i nazwisko, numer Powszechnego Elektronicznego Systemu Ewidencji

Ludności (PESEL), jeżeli został nadany, oraz ostatni adres spadkodawcy,

1) imię i nazwisko, numer PESEL albo numer w Krajowym Rejestrze
...
1) pouczenie składającego wykaz inwentarza co do obowiązku jego

uzupełnienia, w przypadkach wskazanych w art. 10311
...
. Wartość przedmiotu zapisu windykacyjnego oblicza się według stanu
...
1) mają przygotowanie zawodowe do prowadzenia produkcji rolnej, albo
1) są małoletni bądź też pobierają naukę zawodu lub uczęszczają do szkół, albo
1) są trwale niezdolni do pracy.
...
1) Utracił moc z dniem 14 lutego 2001 r. w zakresie, w którym odnosi się do spadków otwartych od dnia
...
1) bądź są małoletni, bądź też pobierają naukę zawodu lub uczęszczają do szkół,
